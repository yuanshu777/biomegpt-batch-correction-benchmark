...
method_id <- "mmguide_resgrl_lam02_anchor01_pres2_cov005_delta05"
...
matrices <- list(
  ...
  `Best residual GRL` = read_abundance_csv(
...
study_plot <- ggplot2::ggplot(
  ...
  ggplot2::labs(
    ...
    subtitle = "Colored by study; each panel is PCA on sqrt relative abundance",
    x = "PC1",
    y = "PC2",
    color = "Study"
  ) +
...
condition_plot <- ggplot2::ggplot(
  ...
  ggplot2::labs(
    title = "CRC abundance PCA using MMUPHin benchmark transform",
    subtitle = "Colored by CRC/control condition; each panel is PCA on sqrt relative abundance",
    x = "PC1",
    y = "PC2",
    color = "Condition",
    shape = "Condition"
...
study_path <- file.path(output_dir, "raw_mmuphin_best_residual_grl_pca_by_study_panel.png")
condition_path <- file.path(
  output_dir,
  "raw_mmuphin_best_residual_grl_pca_by_condition_panel.png"
)
score_path <- file.path(output_dir, "raw_mmuphin_best_residual_grl_pca_scores.csv")
